...
public class AddMoneyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length != 2) {
            sender.sendMessage(Survival.getPlugin().colorize(ConfigManager.getErrorPrefix() + "Usage: /addmoney <player|*> <amount>"));
            return true;
        }
        ...
        if (args[0].equalsIgnoreCase("*")) {
            ...
            sender.sendMessage(Survival.getPlugin().colorize(ConfigManager.getMainPrefix() + "Added " + CurrencyManager.formatCurrency(amount) + " to all players."));
        } else {
            OfflinePlayer player = Bukkit.getOfflinePlayer(args[0]);
            if (player.hasPlayedBefore() || player.isOnline()) {
                CurrencyManager.getEconomy().depositPlayer(player, amount);
                sender.sendMessage(Survival.getPlugin().colorize(ConfigManager.getMainPrefix() + "Added " + CurrencyManager.formatCurrency(amount) + " to " + player.getName()));
            } else {
